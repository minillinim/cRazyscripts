...
my $in = new Bio::SearchIO(-format => $options->{'f'}, -file => $options->{'i'} ) ;
    ...
    while( my $hit = $result->next_hit ) 
    {
        while( my $hsp = $hit->next_hsp ) 
        {
            ...
            if(defined $options->{'l'}) {
                if($hsp->query->length < 300) { next; }
                if($hsp->query->length < $options->{'l'})
                {
                    if( $hsp->length('total') < $hsp->query->length * 0.9) { next; }
                }
                elsif( $hsp->length('total') < $options->{'l'}) {
                    next;
                }
            }
            ...
            if(exists $con_id_2_ident{$result->query_name})
            {
                if($con_id_2_ident{$result->query_name} < $hsp->percent_identity)
                {
                    $con_id_2_ident{$result->query_name} = $hsp->percent_identity;
                    $con_id_2_tax{$result->query_name} =  $hit->name;
                    $con_id_2_len{$result->query_name} = $hsp->length('total');
                }
            }
            else
            {
...
foreach my $key (keys %con_id_2_tax)
{
    print $output $key,"\t",$con_id_2_tax{$key},"\t",$con_id_2_len{$key},"\t",$con_id_2_ident{$key},"\n";
}
